		...
		for form in forms:
			...
			if form["method"].lower().strip() == "post":
				...
				if self.payload in req.text:
					self.XSS_DICT["POST"].update({urljoin(self.url,req.url): str(keys)})
					# print(self.XSS_DICT)
	...
	def get_method_form(self):
		...
		for form in forms:
			...
			if form["method"].lower().strip() == "get":
				...
				if self.payload in req.text:
					print({str(urljoin(self.url,req.url)): str(str(keys))})
					self.XSS_DICT["FORM"].update({str(urljoin(self.url,req.url)): str(str(keys))})
			...
			if url.startswith("http://") is False or url.startswith("https://") is False or url.startswith("mailto:") is False:
				...
				if query != "":
					...
					if self.payload in _respon.text or self.payload in self.session.get(query_all).text:
						...
						# print(query_all)
						#print(self.payload)
						#print({"url": base, "payload": self.payload, "full_url": str(query_all)})
						self.XSS_DICT["GET"].update({"url": base, "payload": urllib.parse.quote(self.payload), "full_url": str(query_all)})
						#print(self.XSS_DICT)
